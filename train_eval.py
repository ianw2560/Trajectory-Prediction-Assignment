...
def get_cls_label(gt, motion_modes, soft_label=True):
    ...
    motion_modes = motion_modes.reshape(motion_modes.shape[0], -1).unsqueeze(
        0
    )  # [1 K pred_len*2]
...
def train(
    ...
    for i, (ped_traj, nei_traj, mask_nearest) in enumerate(train_dataloader):
        ...
        pred_traj, scores = model(
            ...
            motion_modes.cuda(),
            mask_nearest.cuda(),
            closest_mode_indices.cuda(),
            num_k=num_k,
            ped_num_k=ped_num_k,
            minADE_loss=minADE_loss,
        )
...
if not os.path.exists(motion_modes_file):
    ...
    motion_modes = get_motion_modes_ours(
        dataset_train,
        args.obs_len,
        args.pred_len,
        args.n_clusters,
...
if args.lr_scaling:
    scheduler = optim.lr_scheduler.MultiStepLR(
        optimizer, milestones=[30, 49], gamma=0.95
    )
...
for ep in range(args.epoch):
    ...
    ade, fde, num_traj, ade_vector, fde_vector = test(
        model, test_loader, motion_modes, args.just_x_y, args.num_k, args.ped_num_k
    )
    if args.lr_scaling:
        scheduler.step()
