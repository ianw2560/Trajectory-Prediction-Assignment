...
def kmeans_(seq, n_clusters=100,num_features=5,x_y=True):
    ...
    if x_y == True:
        ...
        centers = centers.reshape(centers.shape[0],-1,num_features)

    return centers


def trajectory_motion_modes_ours(all_trajs, obs_len, n_units=120, num_features=5, x_y=True):#, smooth_size=3, random_rotation=False):

    if x_y ==True:
        clustering_input = all_trajs[:, obs_len:, 0:2]
    else:
        clustering_input = all_trajs[:, obs_len:]


    # full_ego_trajs [B T 2]
    motion_modes = kmeans_(clustering_input, n_units,num_features, x_y)
    return motion_modes


def get_motion_modes_ours(dataset, obs_len, pred_len, n_clusters, dataset_path, dataset_name, num_features, just_x_y):
    ...
    for i in range(len(dataset)):
        ...
        trajs_list.append(dataset[i][0])

    all_trajs = np.stack(trajs_list, axis=0)

    motion_modes = trajectory_motion_modes_ours(all_trajs, obs_len, n_units=n_clusters, num_features=num_features, x_y= just_x_y)#,smooth_size=smooth_size, random_rotation=random_rotation)
    
    if not os.path.exists(dataset_path): 
        os.makedirs(dataset_path)

    save_path_file = dataset_path + dataset_name + str(n_clusters)+str(num_features) + str(just_x_y) + '_motion_modes.pkl'
